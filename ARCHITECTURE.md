...
The annotated file tree for this project is maintained separately to keep it accurate and up to date:

- **`agent-action-files.md`** — generated by the planning agent; authoritative source for AI agents. Contains the full file tree with purpose annotations for all key files.
- **`FILES.md`** — hand-maintained index; source of truth for human readers when `agent-action-files.md` is absent.

Agents and skills should read `agent-action-files.md` first, fall back to `FILES.md`, and only search `src/` directly if neither file exists.
